<template>
  ...
  <div v-if="filminfo">
    ...
    <van-divider />
    <van-cell title="剧照" is-link :value="`全部${alllength}`" />
...
// Vue.use(vueToTop)
import { ImagePreview, Toast } from 'vant' // 日期格式化
...
export default {
  ...
  mounted () {
    ...
    http({
      ...
      this.alllength = this.filminfo.photos.length
      console.log(this.filminfo.photos.length)
